...
async function main() {
    try {
        const nvidiaData = await getFlowObjectFromLLM("How are you doing?");
        // console.log(nvidiaData);
        console.log(JSON.stringify(nvidiaData, null, 2));
    } catch (error) {
        console.error('Error running main function:', error.message);
    }
}

// Create a JavaScript object representing the flow graph
function createFlowObject(prompt,response) {
    flowObject = {
        nodes: [
            {id: "six_api", type: "default", position: {x: 0, y: 0}, data: {label: prompt}},
            {id: "final_response", type: "default", position: {x: 100, y: 0}, data: {label: response || "missing response"}}
        ],
        edges: [
            {id: "e1-2", source: "six_api", target: "final_response", type: "smoothstep"}
        ]
    };
    return flowObject;
}
...
// This can be called to retrive the graph
async function getFlowObjectFromLLM(prompt) {
    try {
        const response = await llm(prompt);
        return createFlowObject(prompt,response.content);
    } catch (error) {
        console.error('Error getting flow object from LLM:', error.message);
        return createFlowObject("Error: " + error.message);
    }
}
...
// Example usage
// const flowObject = createFlowObject("hi");
